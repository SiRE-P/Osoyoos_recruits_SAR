...
# read.csv("../data/Columbia_Sockeye_Dam_Counts_Adj_240908.csv") %>%            ### <-- requires prior execution of "Download Sockeye Dam Counts from CBR-DATA.R"
  ...
  janitor::clean_names()                                                        ###     unless version 240913 exists in DATA subdirectory already
...
deadpitch_prop_natural <- ok_riv_deadpitch %>%                      # get proportion natural by spawning ground section and year
  ...
  mutate(osoyoos_stocked = ifelse(year == 2013, TRUE, FALSE))%>%    # BOTH Osoyoos and Skaha were stocked, approx evenly, in 2010 (from BY2009), mainly returning in 2013
  mutate(osoyoos_stocked = ifelse(section == "lower_ok_river",      # osoyoos_stocked variable does not apply to mid- or upper river spawners,
                                  osoyoos_stocked, NA))%>%          # so set variable to NA for those sections
  arrange(section, year)                                          ### NOTE ALSO: all H-O fish released in 2012 & 2013 were into Osoyoos; returning mainly in 2015 and 2016 as 1.2s but also 2014 as 1.1s and 2017 as 1.3s <-- not sure if that is handled (or needs to be?) in this procedure!!?


# deadpitch_lower_ok_river <- ok_riv_deadpitch %>%                    # re-analysis [241025] based on cleaned up DP data; by spawning ground Section...
#   filter(section == "lower_ok_river") %>%                           # process lower Ok River data...
#   filter(species_from_age == "sockeye") %>%     
#   filter(hatchery != "unknown") %>%
#   mutate(origin = hatchery) %>%
#   dplyr::select(section, year, origin, ona_number) %>% 
#   group_by(section, year) %>% 
#   summarize(n_natural = sum(origin == "natural"), n = n()) %>% 
#   mutate(p_nat = n_natural / n) %>%                                 # calculate proportion N-O (natural)
#   mutate(osoyoos_stocked = ifelse(year == 2013, TRUE, FALSE))%>%    # BOTH Osoyoos and Skaha were stocked, approx evenly, in 2010 (from BY2009), mainly returning in 2013
#   arrange(section, year)                                          ### NOTE ALSO: all H-O fish released in 2012 & 2013 were into Osoyoos; returning mainly in 2015 and 2016 as 1.2s but also 2014 as 1.1s and 2017 as 1.3s <-- not sure if that is handled (or needs to be?) in this procedure!!?
# 
# deadpitch_middle_ok_river <- ok_riv_deadpitch %>%                   # process middle Ok River data...
#   filter(section == "middle_ok_river") %>%
#   filter(species_from_age == "sockeye") %>%     
#   filter(hatchery != "unknown") %>%
#   mutate(origin = hatchery) %>%
#   dplyr::select(section, year, origin, ona_number) %>% 
...
# 
# deadpitch_upper_ok_river <- ok_riv_deadpitch %>%                    # process upper Ok River data...
#   filter(section == "upper_ok_river") %>%
#   filter(species_from_age == "sockeye") %>%     
#   filter(hatchery != "unknown") %>%
#   mutate(origin = hatchery) %>%
#   dplyr::select(section, year, origin, ona_number) %>% 
...
#   arrange(section, year) 
# 
# deadpitch_shingle_creek <- ok_riv_deadpitch %>%                     # process Shingle Creek trib to upper Ok River data...
#   filter(section == "shingle creek") %>%
#   mutate(section = "shingle_creek") %>%
#   filter(species_from_age == "sockeye") %>%     
#   filter(hatchery != "unknown") %>%
#   mutate(origin = hatchery) %>%
#   dplyr::select(section, year, origin, ona_number) %>% 
...
# deadpitch_upper_ok_river <-ok_riv_deadpitch %>%                     # original analysis by Section
#   filter(section == "upper_ok_river") %>%                                               
# # deadpitch_pen <- upper_ok_river_deadpitch %>%                     # process upper Ok River (Pen Channel) deadpitch data...
#   filter(species_from_age == "sockeye") %>%                         # Check whether this captures "Sockeye" too [not necessary, all lower case]
#   filter(thermal != "NA") %>% 
...
# deadpitch <- bind_rows(deadpitch_low_mid, deadpitch_pen,            # combine deadpitch data
#                        deadpitch_shingle, 
#                        deadpitch_2022_2023) %>%                   ### hard-wired data
# # dplyr::filter(section != "Shingle Creek") %>%                   ### drop Shingle Creek from this analysis - insufficient data # commented out 240927 for testing
#   arrange (section, year)   

# deadpitch <- bind_rows(deadpitch_lower_ok_river, 
#                        deadpitch_middle_ok_river, 
#                        deadpitch_upper_ok_river, 
#                        deadpitch_shingle_creek) %>%
#   arrange(section, year)

### still no 2023 data for mid/upper/shingle?
### should these gams omit 2015 and 2016 too?  since they would be
#run cross validation to determine df of year spline
cv_gam_loo(data = deadpitch %>% filter(section == "lower_ok_river") %>% filter(year!=2013), formula = p_nat ~ bs(year, 3))
cv_gam_loo(data = deadpitch %>% filter(section == "lower_ok_river") %>% filter(year!=2013), formula = p_nat ~ bs(year, 4))
cv_gam_loo(data = deadpitch %>% filter(section == "lower_ok_river") %>% filter(year!=2013), formula = p_nat ~ bs(year, 5))
cv_gam_loo(data = deadpitch %>% filter(section == "lower_ok_river") %>% filter(year!=2013), formula = p_nat ~ bs(year, 6))
#lowest MSE is for model with 3 df on spline (was 4 df before 2022 & 2023 estimates added [HS 24.07.04]) 

cv_gam_loo(data = deadpitch %>% filter(section == "middle_ok_river") %>% filter(year!=2013), formula = p_nat ~ bs(year, 3))
cv_gam_loo(data = deadpitch %>% filter(section == "middle_ok_river") %>% filter(year!=2013), formula = p_nat ~ bs(year, 4))
cv_gam_loo(data = deadpitch %>% filter(section == "middle_ok_river") %>% filter(year!=2013), formula = p_nat ~ bs(year, 5))
cv_gam_loo(data = deadpitch %>% filter(section == "middle_ok_river") %>% filter(year!=2013), formula = p_nat ~ bs(year, 6))
#lowest MSE is for model with 5 df on spline (same after 2022 & 2023 estimates added [HS 24.07.24])

cv_gam_loo(data = deadpitch %>% filter(section == "upper_ok_river") %>% filter(year>2011), formula = p_nat ~ bs(year, 3))
cv_gam_loo(data = deadpitch %>% filter(section == "upper_ok_river") %>% filter(year>2011), formula = p_nat ~ bs(year, 4))
cv_gam_loo(data = deadpitch %>% filter(section == "upper_ok_river") %>% filter(year>2011), formula = p_nat ~ bs(year, 5))
cv_gam_loo(data = deadpitch %>% filter(section == "upper_ok_river") %>% filter(year>2011), formula = p_nat ~ bs(year, 6))
...
# dplyr::filter(section != "Shingle Creek") +
  facet_wrap(~section, ncol=1)+
  scale_x_continuous(breaks = c(seq(from = 2007, to = last_year, by = 2)))+
...
# filter(spawning_location != "Penticton Channel") %>%    # Pen Chan data now not omitted from calculation of proportion OSO strays - see new SUMMARISE stmt below]
...
ggplot(columbia_okanagan_long, aes(x=Year, y = Recruits/1000, color = Group))+  # multi-line plot of Columbia Sockeye recruits by sub-group
  geom_line() +
...
okanagan_recruits_long <- columbia_okanagan_long %>%                            # set up data for stacked bar plot 
  ...
  group_by(Year) %>%
  mutate(Group = replace(Group, Group == "Osoyoos_Natural", "1_Osoyoos_Natural")) %>%
  mutate(Group = replace(Group, Group == "Skaha_Natural",   "2_Skaha_Natural"))   %>%
  mutate(Group = replace(Group, Group == "Hatchery_Origin", "3_Hatchery_Origin")) %>%
  mutate(Proportion = Recruits / sum(Recruits))

ggplot(okanagan_recruits_long,                                                  # stacked bar plot of Okanagan Sockeye recruits by sub-group
...
columbia_okanagan_recruits %>% ggplot(aes(x = return_year, y = oso_nat_recruits/1000))+
  ...
  geom_point(size = 2)+
  geom_segment(aes(x = 1977,   xend = 2007.5, y = pre_fwmt,  yend = pre_fwmt),  colour = "red",       linewidth = 1, linetype = 2)+ # CB: this is for the option of adding the pre- and current freshwater management tools era 
  geom_segment(aes(x = 2007.5, xend = 2023,   y = post_fwmt, yend = post_fwmt), colour = "darkgreen", linewidth = 1, linetype = 2)+ #     to compare median recruits between the two time periods
  geom_hline(yintercept = geometric.mean(columbia_okanagan_recruits$oso_nat_recruits)/1000,colour= "blue", linewidth = 1, linetype = 2)+
