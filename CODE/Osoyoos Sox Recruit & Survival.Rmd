...
#lowest mean square error is for model with 3 df on spline (was 4 df before 2022 & 2023 estimates added [HS 24.07.04]) 
...
#lowest mean square error is for model with 5 df on spline (same after 2022 & 2023 estimates added [HS 24.07.24])
...
#lowest Pen Channel mean square error is for model with 4 df on spline [HS 24.07.24]
...
pnat_pen_m <- gam(p_nat ~ bs(year, 5), data = deadpitch %>% filter(section == "Penticton Channel") %>% filter(year>2011),family = binomial(link = "logit"))
...
prop_oso_strays <- prop_oso_strays_by_year %>% 
  ...
  bind_rows(data.frame(spawning_location = "Lower Ok River", year = 2011,            # apply mean stray rate to later years (was 2013:2021)
                       prop_oso_strays = prop_oso_strays_mean)) %>%
  arrange(year)
```  
...
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2006, year < 2022), formula = prop_oso_origin_nat ~ bs(year, 3)) # years 2022 and 2023 not yet ready for analysis as spawners data not up-to-date [hs 240705]
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2006, year < 2022), formula = prop_oso_origin_nat ~ bs(year, 4)) # ditto
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2006, year < 2022), formula = prop_oso_origin_nat ~ bs(year, 5))
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2006, year < 2022), formula = prop_oso_origin_nat ~ bs(year, 6)) # 6 df gives lowest mean square error
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2006, year < 2022), formula = prop_oso_origin_nat ~ bs(year, 7))

#filtering out early years where the proportion is 0
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2011, year < 2022), formula = prop_ska_origin_nat ~ bs(year, 3)) # years 2022 and 2023 not yet ready for analysis as spawners data not up-to-date [hs 240705]
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2011, year < 2022), formula = prop_ska_origin_nat ~ bs(year, 4)) # 4 df gives lowest mean square error
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2011, year < 2022), formula = prop_ska_origin_nat ~ bs(year, 5))
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2011, year < 2022), formula = prop_ska_origin_nat ~ bs(year, 6))
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2011, year < 2022), formula = prop_ska_origin_nat ~ bs(year, 7))

prop_oso_natural_m <- gam(prop_oso_origin_nat ~ bs(year, 6), data = spawning_totals_no2011 %>% 
                            filter(year > 2006, year < 2022), family = binomial(link = "logit"))  # OSO imputation model ### restricted to <2022 ?
prop_ska_natural_m <- gam(prop_ska_origin_nat ~ bs(year, 4), data = spawning_totals_no2011 %>% 
                            filter(year > 2010, year < 2022), family = binomial(link = "logit"))  # SKA imputation model ### restricted to <2022 ?
...
# filter(year < 2022) %>%   # filter was probably needed to omit missing 2022 and 2023 years...  [hs 240801]
  arrange(year)
...
#so_recruits$X <- NULL                                                                                # drop X (extra observation counter?)

oso_recruits <- annual_returns %>%
  left_join(spawning_totals %>% dplyr::rename(return_year = year)) %>% 
  mutate(prop_oso_origin_nat  = ifelse(is.na(prop_oso_origin_nat), 1, round(prop_oso_origin_nat,3))) %>% 
  mutate(prop_oso_origin_nat  = ifelse(return_year == 2022, 0.719, prop_oso_origin_nat)) %>%          ### Hardwire Prop OSO for 2022 till raw input data are available
  mutate(prop_oso_origin_nat  = ifelse(return_year == 2023, 0.542, prop_oso_origin_nat)) %>%          ### Hardwire Prop OSO for 2023 till raw input data are available
  mutate(prop_ska_origin_nat  = ifelse(is.na(prop_ska_origin_nat), 0, round(prop_ska_origin_nat,3))) %>%
  dplyr::select(return_year, total_harvest_below_bonn, bon_sox, snake_sox, mid_columbia_sox, 
                ok_stock_comp, oka_sox, prop_oso_origin_nat, prop_ska_origin_nat,  
                oso_strays, ska_strays) %>%
  mutate(oso_strays = round(oso_strays, 0), ska_strays = round(ska_strays, 0)) 


oso_recruits.df <- oso_recruits %>% 
  mutate(prop_oso_strays = round(prop_oso_strays,3)) %>%                                             # rounding off props and totals from spawning ground data...
  mutate(prop_ska_strays = round(prop_ska_strays,3)) %>%                                                        
  ...
  mutate(okanagan = oka_sox) %>%                                                                               # Oka-bound SK at mouth of Columbia
  mutate(wenatchee = round(mid_columbia_sox * (1 - ok_stock_comp), 0)) %>%                                     # Wen-bound SK at mouth of Columbia
  mutate(oso_origin_nat_returns = round(okanagan * prop_oso_origin_nat, 0) + oso_strays - ska_strays) %>%      # OSO recruits
  ...
  mutate(ska_total_returns = ifelse(return_year == 2013, ska_origin_nat_returns + 0.5 * hatchery_returns,      # SKA total = H-O + N-O (presuming 50% of H-O returns in RY2013 --> SKA (since half of BY2009 releases were in SKA and half in OSO lake...
                                    ifelse(return_year %in% c(2015, 2016, 2017), ska_origin_nat_returns,       #                        whereas in RY2015 & 2016, all H-O returns were of OSO origin, so add none to Skaha...
...
filename <- paste("../figures/Okanagan_Recruits_", timestamp, ".png", sep = "") # filename for figure output 
ggsave(file = filename, width = 6, height = 10, units = "in")                   # saves the plot to filename

# filename <- paste("../output/_Recruits_", timestamp, ".csv", sep = "")        # filename for recruitment output 
...
columbia_recruits <- oso_recruits.df %>%
  dplyr::select(return_year, bon24_counts = bon_sox, 
                harv_downstream = total_harvest_below_bonn, snake_sox,
                mid_columbia_sox, ok_stock_comp, oka_sox, prop_oso_origin_nat,
                prop_oso_strays, oso_origin_nat_returns)

filename <- paste("../output/Columbia_Recruits_", timestamp, ".csv", sep = "")  # filename for recruitment output 
write.csv(columbia_recruits, filename)                                          # saves the data to filename
                
# create timeseries of return year and number of total okanagan, osoyoos & skaha sub-stock recruits, and write to a CSV file
oka_recruits <- oso_recruits.df %>% 
  dplyr::select(return_year, columbia=columbia_sox, okanagan, 
                oso_origin_nat_returns, ska_origin_nat_returns, 
                hatchery_returns, ska_total_returns)
...
# NOTE that oso_sox_mouth here is the number of osoyoos recruits, not the total number of sockeye of all stocks below Bonneville
