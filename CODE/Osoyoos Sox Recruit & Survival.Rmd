...
ggsave(file = filename, width = 8, height = 8, units = "in")                    # saves the plot to filename
...
okanagan_recruits_long <- columbia_okanagan_long %>%                            # set up data for stacked bar plot 
  ...
  group_by(Year) %>%
  mutate(Group = replace(Group, Group == "Osoyoos_Natural", "3_Osoyoos_Natural")) %>%
  mutate(Group = replace(Group, Group == "Skaha_Natural",   "2_Skaha_Natural")) %>%
  mutate(Group = replace(Group, Group == "Hatchery_Origin", "1_Hatchery_Origin")) %>%
...
mar_surv_table <- oso_marine_survival_by_source %>%                             # "Best" Marine Survival selected for output for report table 
  dplyr::select(Smolt_Year = smolt_year, 
                Presmolts = oso_juv_presmolt, 
                Adults = oso_sox_best,
                Survival = oso_sox_survival)
