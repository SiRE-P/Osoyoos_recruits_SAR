...
radat  <- left_join(oso_recruits.df %>% 
                    ...
                    age_data, by = "return_year") %>%            #joining the recruitment dataframe with the age composition data frame 
  mutate(ocean_age       = as.numeric(str_sub(Age, start = -1)),
         s_year          = return_year - ocean_age,
         sox_age_critfc  = oso_sox_mouth * prop_Fryer,           #calculating the number of recruits according to each age composition source
         sox_age_sire    = oso_sox_mouth * prop_SIRE,
         sox_age_best    = oso_sox_mouth * ifelse(best_source == "CRITFC", prop_Fryer, prop_SIRE))     
  radat[is.na(radat)] <- 0 

radat2 <- radat %>%
  group_by(s_year) %>% summarize(oso_sox_critfc = sum(sox_age_critfc),          #summing the recruits by smolt year
  ...
  dplyr::select(s_year, critfc_surv, sire_surv, best_surv) %>%                  #streamlining the dataset to the relevant columns
  pivot_longer(cols = ends_with("_surv"),                                       #arranging the data to long format for plotting
               names_to = "source",
               values_to = "survival")

#plotting marine survival
ggplot(radat2, aes(x = s_year, y = survival, colour = source))+
