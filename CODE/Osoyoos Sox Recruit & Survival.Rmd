...
workbook <- "Snake River Sockeye from Joint Staff Report.xlsx"                  # estimated Snake River Sockeye component from WDFW/ODFW Joint Staff Report
worksheet<- "Snake River Sockeye Estimates"                                     # Table 15
filename <- paste("../data/", workbook, sep = "")
...
returns_data <- readxl::read_xlsx("../data/SIS - Data - OSOYOOS - 24.06.24.xlsx", sheet = "Sockeye Returns Data") %>%  # latest SIS workbook - update to Total Harvest (all fisheries) below BON
  ...
  dplyr::select(return_year, tot_bonn_16hr = total_sox_bonn_16_hour, total_sockeye_snake, total_comm_harvest_below_bonn, 
                total_sockeye_rock_i_24_hr, total_sockeye_rrh_24_hour, total_sockeye_tum_24_hour, total_sockeye_wells_24_hour,
...
# bonn_rocki_check <- oso_recruits %>%                                                                # check for years where RI24 > BON24
#   dplyr::select(return_year, tot_bonn_16hr, bonn_dam_est, total_sockeye_rock_i_24_hr) %>%
#   mutate(dam_diff <- bonn_dam_est - total_sockeye_rock_i_24_hr)                                     # minor diffs in 1995 & 1999
###
