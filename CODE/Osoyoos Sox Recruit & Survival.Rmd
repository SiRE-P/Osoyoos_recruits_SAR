...
dam_counts <- Columbia_Sockeye_Dam_Counts_Adj %>%                               ### <-- requires prior execution of "Download Sockeye Dam Counts from CBR-DATA.R"
  dplyr::select(return_year = Return_Year,   bon_sox = Bonn_Sockeye_adj, 
                rid_sox = RockI_Sockeye_adj, rrh_sox = RRH_Sockeye_adj, 
                tum_sox = Tum_Sockeye_adj,   wel_sox = Wells_Sockeye_adj)

stock_comp <- Columbia_Sockeye_Stock_Comp %>% 
  dplyr::select(return_year = Return_Year, ok_stock_comp = Ok_Stock_Comp_Best)
...
#               osoyoos_natural_origin_no_strays, skaha_origin_returns) %>% 
  dplyr::select(return_year, columbia=columbia_sox, okanagan, oso_origin_nat_returns, ska_origin_nat_returns, hatchery_returns) %>% 
...
age_proportions_ONA_CRITFC <- age_comp_ONA_CRITFC %>%                        ### see/run "Age Composition - ONA vs CRITFC.R" for source of age_comp_ONA_CRITFC ###
