...
AUC_data_Import <- read.csv("../data/AUC_data_Import_xlsx_241105_AO.csv")         # AO checking nerkid estimates and providing updates (241105)    # keep for GitHub [240510]
...
### fill in some total_sockeye blanks where no %sox in deadpitch available but we can assume 100% sockeye for the few numbers of fish from AUC 
  ...
  mutate(sockeye_est   = ifelse(!is.na(total_sockeye), "Observed", "Unknown")) %>%
  ...
  # e.g., no deadpitch in mOkR in 2009, 2016-2022, but few fish, assume 100% Sockeye...
...
ggplot(spawning_location_compare, aes(x=year)) +
  ggtitle("Proportion Sockeye vs Kokanee") +
  facet_wrap(~spawning_location, ncol=2) + #, scales="free_y") +
  geom_line (aes(y = p_sock_deadp), color = "black") + #, size=1) +
  geom_point(aes(y = p_sock_deadp, color = sockeye_est)) +
  labs(x=NULL, y="Percent Sockeye") +
  scale_x_continuous(breaks = c(seq(from = 2000, to = last_year, by = 4)))+
  # scale_color_manual(values = c("Sockeye %" = "blue", "Old Sockeye %" = "gray")) +
  theme_minimal() +
  labs(color = "Sockeye Estimate:") +  
  theme_pt(major_grid=TRUE) +
  theme(legend.position = "bottom", legend.title = element_text(size=9),
        legend.background = element_rect(fill="white", colour="grey80", linewidth = 0.5))
...
spawning_totals_no2011 <- spawning_table %>%                                               # tabulate spawning ground data 
