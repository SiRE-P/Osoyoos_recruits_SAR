...
prop_natural <- bind_rows(deadpitch_prop_natural_filtered %>% mutate(type = "Sampled"), 
  ...
  mutate(section_label = ifelse(section == "upper_ok_river",  "Penticton Channel (Upper Okanagan River)", section_label)) %>%
...
ggplot(prop_natural, aes(x = year, y = p_nat))+
  ...
  facet_wrap(~section_label, ncol=1)+
  scale_x_continuous(breaks = c(seq(from = 2007, to = last_year, by = 2)))+
...
deadpitch_prop_sockeye <- ok_riv_deadpitch %>%                        # get proportion Sockeye vs Kokanee by spawning ground section and year
  ...
  mutate(p_sockeye = n_sockeye / n) %>%                               # calculate proportion Sockeye (H-O and N-O)
  mutate(testalinden_spill = ifelse(year == 2013, TRUE, FALSE))%>%  # 2010 Testalinden dam failure might affect juvs in Osoyoos Lk mainly returning in 2013
  mutate(testalinden_spill = ifelse(section == "lower_ok_river",
                                    testalinden_spill, NA))%>%
  arrange(section, year)   
...
# filter(year != 2011)                                                                                 ### but using sub-optimal methods [240221]
  ...
  mutate(rearing_lake = ifelse(spawning_location == "Middle Ok River", "Skaha", rearing_lake)) %>%     # mutate(rearing_lake) assigns spawners in middle channel     
  dplyr::select(section, year, spawning_location, rearing_lake, spawning_nerkids, percent_35cm_fl) %>% # to Skaha (assumed rearing lake of origin) for purposes of
...
spawning_location_compare <- AUC_data %>% 
  ...
  summarise(nerkids = round(sum(spawning_nerkids)),
            ...
            total_sockeye     = round(sum(total_sockeye   ))) %>% 
  arrange (rearing_lake, spawning_location, year)

ggplot(spawning_location_compare, aes(x=year)) +
  ggtitle("Total Sockeye") +
  facet_wrap(~spawning_location, ncol=2, scales="free_y") +
  geom_line(aes(y = total_sockeye), color = "blue") +
  geom_line(aes(y = total_sockeye_old), color = "red") +
  geom_point(aes(y = total_sockeye), color = "blue") +
  geom_point(aes(y = total_sockeye_old), color = "red") +
  labs(x=NULL, y="Total Sockeye") +
  scale_x_continuous(breaks = c(seq(from = 2000, to = last_year, by = 4)))+
  scale_color_manual(values = c("Total Sockeye" = "blue", "Old Total" = "red")) +
  theme_minimal() +
  theme_pt(major_grid=TRUE) +
  theme(legend.position = "bottom", legend.title = element_text(size=9),
        legend.background = element_rect(fill="white", colour="grey80", linewidth = 0.5))
...
ggplot(spawning_location_compare, aes(x=year)) +
  ggtitle("Percent Sockeye vs Kokanee") +
  facet_wrap(~spawning_location, ncol=2) + # , scales="free_y") +
  geom_line(aes(y = p_sock_deadp), color = "blue") +
  geom_line(aes(y = p_sock_len35), color = "red") +
  geom_point(aes(y = p_sock_deadp), color = "blue") +
  geom_point(aes(y = p_sock_len35), color = "red") +
  labs(x=NULL, y="Percent Sockeye") +
  scale_x_continuous(breaks = c(seq(from = 2000, to = last_year, by = 4)))+
  scale_color_manual(values = c("Sockeye %" = "blue", "Old Sockeye %" = "red")) +
  theme_minimal() +
  theme_pt(major_grid=TRUE) +
  theme(legend.position = "bottom", legend.title = element_text(size=9),
        legend.background = element_rect(fill="white", colour="grey80", linewidth = 0.5))
...
spawning_location <- spawning_location_compare %>%
  dplyr::select(rearing_lake, spawning_location, year, nerkids, p_sock_deadp, total_sockeye)
...
prop_oso_strays_by_year <- spawning_location %>%          # calculate OSO stray rate based on years where no SKA N-O returns exist (2009-2012)
  ...
  left_join(prop_natural %>% dplyr::select(year, spawning_location, p_nat)) %>% 
