...
Columbia_Sockeye_Dam_Counts_Adj <- 
...
dam_counts <- Columbia_Sockeye_Dam_Counts_Adj %>%                               
  dplyr::select(return_year,                  bon_sox = bonn_sockeye_adj, 
...
annual_returns <- dam_counts %>%
  ...
  arrange(return_year) %>%
  mutate(bon_sox = round(bon_sox, 0)) %>%
  mutate(columbia_sox = bon_sox + ifelse(is.na(total_harvest_below_bonn), 0, total_harvest_below_bonn)) %>%
  mutate(mid_columbia_sox = columbia_sox - snake_sox) %>%
  mutate(oka_sox = round(mid_columbia_sox * ok_stock_comp, 0)) %>%
...
oka_recruits <- oso_recruits.df %>% 
  dplyr::select(return_year, columbia=columbia_sox, okanagan, oso_origin_nat_returns, ska_origin_nat_returns, hatchery_returns, ska_total_returns)
...
# coord_cartesian(xlim = c(1986, last_year)) +
  scale_x_continuous(breaks = c(seq(from = 1984, to = last_year, by = 4))) +
...
oso_marine_survival <- oso_age_at_return %>%                                    # gathering marine survival terms together for report
  group_by(smolt_year) %>% summarize(oso_sox_critfc = sum(sox_age_critfc),      # summing the recruits by smolt year
                                     oso_sox_sire   = sum(sox_age_sire),
                                     oso_sox_best   = sum(sox_age_best)) %>% 
  left_join(., j_dat, by = "smolt_year") %>%                                    # joining the juvenile dataframe
