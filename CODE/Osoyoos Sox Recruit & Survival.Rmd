...
# read.csv("../data/Columbia_Sockeye_Stock_Comp_240908.csv") %>%                ### <-- requires prior execution of "Download Sockeye Dam Counts from CBR-DATA.R"
  read.csv("../data/Columbia_Sockeye_Stock_Comp_240913.csv") %>%                ### <-- requires prior execution of "Download Sockeye Dam Counts from CBR-DATA.R"
...
deadpitch <- bind_rows(deadpitch_low_mid, deadpitch_pen,            # combine deadpitch data
  ...
  dplyr::filter(section != "Shingle Creek") %>%                     # drop Shingle Creek from this analysis
...
age_proportions_ONA_CRITFC <- age_comp_ONA_CRITFC %>%                        
    ...
    mutate(best_source_new = ifelse(return_year %in% c(1985,1987:2001,2004:2023), # next, create best_source_new assignment variable by setting all years specified here to CRITFC...
                                   "CRITFC", "UNKNOWN")) %>%
