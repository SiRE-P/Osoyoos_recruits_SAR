...
AUC_data_Import <- readxl::read_xlsx("../data/AUC_data_Import_xlsx_2025.06.25_AO.xlsx", sheet = "AUC_data_Import") %>%  # AO updated Pen 2023 and Shingle 2021-2023 %
                   janitor::clean_names() %>% 
...
write.csv(AUC_data_Import,  "../data/AUC_data_Import_250625.csv")        # if AUC_data imported from AO xlsx workbook, re-write to CSV   # comment out for GitHub
auc_data_import <- read.csv("../data/AUC_data_Import_250625.csv")        # export and re-import (NOTE lower-case dataset name) --- keep for GitHub [241128]
...
# left_join(., deadpitch_prop_sockeye,      by = c("section", "year")) %>%
  ...
  # mutate(p_sockeye = ifelse(section %in% c("lower_ok_river", "middle_ok_river"), 1, p_sockeye)) %>%  # turned off by HS 251014 since it is overriding actual species_from_age
