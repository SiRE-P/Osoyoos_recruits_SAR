...
last_year <- 2024     # current last year of returns data handled or available 
...
workbook <- "Columbia River sockeye harvest (Ryan Lothrop WDFW 251014).xlsx"    # raw USA harvest from WDFW and Okanagan (CAN) harvest by gear
...
# read.csv("../data/Columbia_Sockeye_Dam_Counts_Adj_241111.csv") %>%            ### <-- no changes since 240913.csv but 241111 is a benchmark for latest draft report
  read.csv("../data/Columbia_Sockeye_Dam_Counts_Adj_250807.csv") %>%            ### <-- no changes since 241111.csv but 250808 is a benchmark for latest draft report
  janitor::clean_names()                                                        ###     
...
# read.csv("../data/Columbia_Sockeye_Stock_Comp_241111.csv") %>%                ### <-- no changes since 240913.csv but 241111 is a benchmark for latest draft report
  read.csv("../data/Columbia_Sockeye_Stock_Comp_250807.csv") %>%                ### <-- no changes since 241111.csv but 250807 is a benchmark for latest draft report
  janitor::clean_names()                                                        ###     
...
filename <- ("C:/DFO-MPO/OneDrive/OneDrive - DFO-MPO/okanagan_data/deadpitch/combined_and_cleaned/okanagan_deadpitch_adj_2024_species_assignment.xlsx") # incomplete for 2024 but species assigned based on length for upper OkR and Shingle {hs 251015)
ok_riv_deadpitch <- read_xlsx(filename, sheet="okanagan_deadpitch_adj") # read dp data with preliminary 2024 data 


# # fix 2024 which is missing species info
# ok_riv_deadpitch_bak <- ok_riv_deadpitch
# 
# ok_riv_deadpitch <- ok_riv_deadpitch %>%
#   mutate(species = ifelse(year == 2024 & hatchery == "hatchery", "sockeye"
...
deadpitch_prop_natural <- ok_riv_deadpitch %>%                      # get proportion natural by spawning ground section and year
  filter(species_from_age == "sockeye") %>%                         # for Sockeye only; from revised DP data (PT, AO, 240825)
...
# deadpitch_2024 <- data.frame(                                     ### append summary values for 2024 till raw data compiled [250625]
#   year = c(2024, 2024),
#   section = c("upper_ok_river", "shingle_creek"),                   # Penticton Channel and Shingle Creek sections
#   n_natural = c(xxx, xx),                                         ### natural origin count NA from Nerkid Deadpitch Adult.BioData_24.xlsx (251014)
#           n = c(yyy, yyy)) %>%                                    ### total deadpitch from Nerkid Deadpitch Adult.BioData_24.xlsx
#   mutate(p_nat = n_natural / n) %>%
#   mutate(testalinden_spill = NA)


# deadpitch_2023 <- data.frame(                                     ### append summary values for 2023 till raw data compiled [250625]
#   year = c(2023, 2023),
#   section = c("upper_ok_river", "shingle_creek"),                   # Penticton Channel and Shingle Creek sections
#   n_natural = c(405, 40),                                         ### natural origin count from Nerkid (Skaha) Deadpitch Adult.BioData_23.xlsx (250625)
#           n = c(420, 100)) %>%                                    ### total deadpitch from Nerkid (Skaha) Deadpitch Adult.BioData_23.xlsx
#   mutate(p_nat = n_natural / n) %>%                               
#   mutate(testalinden_spill = NA)               
...
#lowest MSE is for model with 5 df on spline, but using that to fit gam to missing Pen section value (2011) yields way high estimate; 
#even applying 4 df works better than 5, but reset to 3 df below in pnat_pen_m statement, as it was before 2023 data was added;  250724
...
# bind_rows(deadpitch_2023) %>%              # wired data no longer needed
...
prop_natural <- bind_rows(deadpitch_prop_natural_filtered %>% mutate(type = "Sampled"), 
                ...
                data.frame(year = c(2019), section = "middle_ok_river", type = "Imputed",
                           p_nat = predict(pnat_mid_m, data.frame(year = c(2016)), type = "response")), #) %>%  ### imputes to zero unless year=c(2016) in this row
                ...
                data.frame(year = c(2023), section = "middle_ok_river", type = "Imputed",
                           p_nat = predict(pnat_mid_m, data.frame(year = c(2016)), type = "response")), #) %>%  ### imputes to zero unless year=c(2016) in this row
                data.frame(year = c(2024), section = "middle_ok_river", type = "Imputed",
                           p_nat = predict(pnat_mid_m, data.frame(year = c(2016)), type = "response")), #) %>%  ### imputes to zero unless year=c(2016) in this row
                
                data.frame(year = c(2011), section = "upper_ok_river", type = "Imputed", 
                           p_nat = predict(pnat_pen_m, data.frame(year = c(2011)), type = "response"))) %>%
...
deadpitch_prop_sockeye <- ok_riv_deadpitch %>%                        # get proportion Sockeye vs Kokanee by spawning ground section and year
  ...
  # filter(hatchery != "unknown") %>%                                 # omit unknown thermal mark class
  mutate(species = species_from_age) %>%                              # define species based on age data (not fork length)
  mutate(origin = hatchery) %>%                                       # define variable origin based on hatchery = "natural" or "hatchery"
...
cv_gam_loo(data = deadpitch_prop_sockeye %>% filter(section == "middle_ok_river") %>% filter(year>2008), formula = p_sockeye ~ bs(year, 4)) # 4df best [241108, 250724]
...
cv_gam_loo(data = deadpitch_prop_sockeye %>% filter(section == "upper_ok_river") %>% filter(year>2011), formula = p_sockeye ~ bs(year, 3)) # 3df best since 2023 data added [250724]
...
deadpitch_prop_sockeye_plus <- bind_rows(deadpitch_prop_sockeye %>% mutate(type = "Observed"),   ### apply imputed estimate for multiple years for mid-OkR
                               ...
                               data.frame(year = c(2024), section = "middle_ok_river", type = "Assumed", 
                               p_sockeye = predict(p_sock_mid_m, data.frame(year = c(2024)), type = "response"))) %>%
                               
                               # data.frame(year = c(2011), section = "upper_ok_river",  type = "Imputed",          ### apply imputed estimate for Pen Ch in 2011
                               # p_sockeye = predict(p_sock_pen_m, data.frame(year = c(2011)), type = "response")), 
                               # data.frame(year = c(2011), section = "upper_ok_river",  type = "Unresolved",       ### apply imputed estimate for Pen Ch in 2011
                               # p_sockeye = NA), 
                               # data.frame(year = c(2023), section = "upper_ok_river",  type = "Imputed",          ### imputation no longer needed for Pen Ch in 2023 [250724]
                               # p_sockeye = predict(p_sock_pen_m, data.frame(year = c(2023)), type = "response"))) %>%
                               # data.frame(year = c(2024), section = "upper_ok_river",  type = "Imputed",          ### imputation no longer needed for Pen Ch in 2023 [250724]
                               # p_sockeye = predict(p_sock_pen_m, data.frame(year = c(2024)), type = "response"))) %>%

# filter(!(year == 2011 & section == "upper_ok_river")) %>%                                                         # Exclude the 2011 upper_ok_river record as too sketch
  mutate(prop_sock_type = type) %>%
  dplyr::select(-type) %>%
  arrange(section, year)

filename <- paste("../output/Species_composition_", timestamp, ".csv", sep = "") # Total Nerkids, Sockeye, prop Sockeye on  Spawning Grounds  
write.csv(deadpitch_prop_sockeye_plus, filename)                                 # before stray rate calculations
...
#UC_data_Import <- readxl::read_xlsx("../data/AUC_data_Import_xlsx_241105_AO.xlsx", sheet = "AUC_data_Import") %>% 
#UC_data_Import <- readxl::read_xlsx("../data/AUC_data_Import_xlsx_250623_AO.xlsx", sheet = "AUC_data_Import") %>%      # back to the original AUC estimates for 2011
#UC_data_Import <- readxl::read_xlsx("../data/AUC_data_Import_xlsx_2025.06.25_AO.xlsx", sheet = "AUC_data_Import") %>%  # AO updated Pen 2023 and Shingle 2021-2023 %
AUC_data_Import <- readxl::read_xlsx("../data/AUC_data_Import_2025.10.15_AO-HS.xlsx",   sheet = "AUC_data_Import") %>%  # AO updated Pen 2023-2024 and Shingle 2023-2024 %
                   janitor::clean_names() %>% 
                   mutate_at("auc", as.numeric) %>%
                   mutate_at("spawning_nerkids", as.numeric) %>%
                   mutate_at("percent_35cm_fl", as.numeric) %>%                                  # THIS VALUE SHOULD MEAN: "%Nerkids < 35 cm in length", i.e., %KOK
                 ...
                 # mutate(spawning_nerkids = ifelse(year == 2011, NA, spawning_nerkids)) %>%     # nerkids in 2011 set to NA
                   mutate(percent_35cm_fl  = ifelse(year == 2011 & spawning_location == "Penticton Channel", NA, percent_35cm_fl)) %>%
                 ...
                 # mutate(spawning_nerkids_note = ifelse(year == 2011, paste("Suspicious survey estimate (", bad_estimate, ")"), spawning_nerkids_note)) %>%
...
write.csv(AUC_data_Import,  "../data/AUC_data_Import_251015.csv")        # if AUC_data imported from AO xlsx workbook, re-write to CSV   # comment out for GitHub
auc_data_import <- read.csv("../data/AUC_data_Import_251015.csv")        # export and re-import (NOTE lower-case dataset name) --- keep for GitHub [241128]
...
# mutate(percent_35cm_fl = ifelse(year == 2023 & section == "upper_ok_river" & is.na(percent_35cm_fl), 0.80, percent_35cm_fl)) %>%   ### Pen Ch. fish not yet ecotyped via age, but now IMPUTED, above
...
spawning_location_compare <- AUC_data %>% 
  mutate(old_prop_sock = 1 - percent_35cm_fl, old_total_sockeye = spawning_nerkids * old_prop_sock) %>%# get  
# left_join(., deadpitch_prop_sockeye,      by = c("section", "year")) %>%
  ...
  mutate(p_sockeye = ifelse(section %in% c("lower_ok_river", "middle_ok_river"), 1, p_sockeye)) %>%    # may be turned off - since it is overriding actual species_from_age
...
spawning_location <- spawning_location_compare %>%                                                 
  dplyr::select(rearing_lake, spawning_location, year, nerkids, p_sock_deadp, total_sockeye, sockeye_est) # trim dataframe to revised estimates

filename <- paste("../output/Spawning_Basics_", timestamp, ".csv", sep = "")    # Total Nerkids, Sockeye, prop Sockeye on  Spawning Grounds  
write.csv(spawning_location, filename)                                          # before stray rate calculations
...
spawning_table <- spawning_location %>%                                                    # populate Spawning Table from AUC count of spawners in...   
  ...
  mutate(section = ifelse(spawning_location == "Penticton Channel", "upper_ok_river",  section)) 
# filter(spawning_location != "Shingle Creek")                                             # Shingle Creek omitted in next step (spawn_table_summary) because of low numbers & data quality

filename <- paste("../output/Spawner_Estimates_", timestamp, ".csv", sep = "")             # Total Nerkids, Sockeye, prop Sockeye on  Spawning Grounds  
write.csv(spawning_table %>% dplyr::select(-section), filename, row.names = FALSE)                                                        # after stray rate calculations
...
spawning_totals <- spawning_totals_no2011 %>%
  mutate(prop_natural_origin_type = "sampled") %>%
  arrange(year)
...
j_dat   <- read.csv("../data/juv_dat_ATS_251014.csv")     # ATS-based juv (PRESMOLT) estimates from SIS-OSO xlsx workbook - not spline-based model estimates of SMOLTS (see SIS-OSO xlsx MASTER)
