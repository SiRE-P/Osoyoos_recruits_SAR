...
# update: 240515 - 
# update: TO DO  - incorporate RY2022 and 2023 data to extend mar-surv time-series to SY2021
#####
...
# returns_data <- readxl::read_xlsx("./data/SIS - Data - OSOYOOS - 24.05.13.xlsx", sheet = "Sockeye Returns Data") %>%   # latest SIS workbook - update to Total Harvest (all fisheries) below BON
# janitor::clean_names()                                                                                                 # (which is 24.05.13.xlsx as of 240515)
# returns_select <- returns_data %>%                                                                                     # input BON dam counts, Snake abund., d/s harvest & stock composition %
# filter(!is.na(return_year), return_year <= last_year) %>%
# dplyr::select(return_year, tot_bonn_16hr = total_sox_bonn_16_hour, total_sockeye_snake, ok_stock_comp_percent_best, total_comm_harvest_below_bonn)
# write.csv(returns_select,"../data/returns_select_SIS240212_xlsx_240516.csv")                                          # if SIS xlsx data imported, re-write to CSV

returns_select <- read.csv("../DATA/returns_select_SIS240212_xlsx_240516.csv")                                        # replaces import data from SIS workbook [240510]
...
# linear_eqn_f <- function(x,y) {                # stolen function to be called from ggplot to add linear reg stats to plots...
#   m <- lm(y ~ x)                               # but it's not working... 
...
stats_txt = "ln(Y) = -4.89 + 1.15 * ln(X); r = 0.98; deviance = 18.2"                 # stats to annotate regression plot   
...
# geom_jitter(height=1) +
  ...
  geom_text(x=0, y=35, label=stats_txt, hjust="inward", size=4) +
  labs(title="Bonneville Dam Sockeye Counts", subtitle = "1995-2002, 2013-2022")+
  ylab("24 hr - 16 hr Count Difference (1000s)")+
  xlab("16 hr Count (1000s)")+
  theme_bw()+
...
#   janitor::clean_names()                                                                       # detection of thermal marks                                # comment out for GitHub
# write.csv(ok_riv_deadpitch,  "../data/ok_riv_deadpitch_xlsx_240516.csv")                        # if OSO SOX deadpitch xlsx data imported, re-write to CSV  # comment out for GitHub
ok_riv_deadpitch <- read.csv("../data/ok_riv_deadpitch_xlsx_240516.csv")                        # replaces import data from OSO SOX deadpitch xlsx workbook # keep for GitHub [240510]
...
ggplot(prop_natural, aes(x = year, y = p_nat))+
  ...
  geom_line()+
  facet_wrap(~section)+
  scale_x_continuous(breaks = c(seq(from = 2000, to = 2022, by = 2)))
...
# AUC_data_Import <- readxl::read_xlsx("./data/SIS - Data - OSOYOOS - 24.02.12.xlsx", sheet = "OK AUC and Deadpitch Results") %>%                               # comment out for GitHub
#   janitor::clean_names()                                                                                                                                      # comment out for GitHub  
# write.csv(AUC_data_Import,  "../data/AUC_data_Import_xlsx_240516.csv")                        # if AUC_data imported from SIS xlsx workbook, re-write to CSV   # comment out for GitHub
AUC_data_Import <- read.csv("../data/AUC_data_Import_xlsx_240516.csv")                        # replaces AUC data from SIS xlsx workbook                       # keep for GitHub [240510]
...
stray_correct <- function(obs_focal, obs_other, stray_focal, stray_other){
  ((stray_other - 1) * obs_focal + stray_other * obs_other)/(stray_focal + stray_other - 1)}
...
### HS: not sure if this helps or is necessary. so commenting out CB's fixes for a "negative number" [240426]
### CB: giving 2011 a value > 0 so GAM with the binomial distribution being used to impute 2011 can have an input value that isn't negative
### spawning_totals[spawning_totals$year %in% c(2011), 9]  <- c(rep(0, 1))                     # set ska_nat_origin = 0 (instead of -1,478 which was due to stray rate) [HS 240416]
### spawning_totals[spawning_totals$year %in% c(2011), 13] <- c(rep(0.0000000001, 1))          # set prop_ska_origin_nat = 0 or so [CB 240416]

### HS: these plots are captured in last step in this chunk, so commented out here [240426]
# spawning_totals %>% 
#   ggplot(aes(x = year, y = prop_ska_origin_nat))+
#   geom_point()+
#   geom_line()+
...
# 
# spawning_totals %>% 
#   ggplot(aes(x = year, y = prop_oso_origin_nat))+
#   geom_point()+
#   geom_line()+
#   geom_smooth(method = "gam", formula = y ~ bs(x, 6), method.args = list(family = binomial(link = "logit")), se = FALSE)+
#   scale_x_continuous(breaks = c(seq(from = 2000, to = last_year, by = 4)))
...
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2006), formula = prop_oso_origin_nat ~ bs(year, 6))
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2006), formula = prop_oso_origin_nat ~ bs(year, 7))
#6 df gives lowest mean square error
...
cv_gam_loo(data = spawning_totals_no2011 %>% filter(year > 2011), formula = prop_ska_origin_nat ~ bs(year, 4))
...
#4 df gives lowest mean square error
...
write.csv(spawning_totals, "../output/Spawning_Totals_HS_240516.csv")
...
# left_join(bonn_pred  %>% dplyr::select(return_year = year, bonn_dam_est = bonn_24hr)) %>%           # Bonn 24-hr estimates using predicted values for all years...
  ...
  mutate(total_comm_harvest_below_bonn =                                                              # this var is actually ALL harvest below BON (REC, COMM, TREATY) - data update (Ryan Lothrop WDFW May 2024)
         ifelse(is.na(total_comm_harvest_below_bonn), 0, total_comm_harvest_below_bonn))              # set missing harvest to 0
                                                                                                      # revisit decision to set harvest=0 when NA  
oso_recruits.df <- oso_recruits %>% 
  ...
  mutate(okanagan = round(columbia * ok_stock_comp_percent_best,0)) %>%                                        # Ok-bound SK at mouth of Columbia
  ...
  mutate(prop_ska_strays = round(prop_ska_strays,3)) %>%                                                       # round off column data... 
...
write.csv(oso_recruits.df, file = "../output/recruits_deterministic_240516.csv")                       # output recruitment data

#CB: mean before and after FWMT implementation - note, 2008 is first year where we might expect to see difference in adult returns
pre_fwmt  <- mean(oso_recruits.df[oso_recruits.df$return_year %in% c(1977:2007),]$oso_origin_nat_returns)/1000
post_fwmt <- mean(oso_recruits.df[oso_recruits.df$return_year %in% c(2008:last_year),]$oso_origin_nat_returns)/1000

#CB: plotting recruits through time
oso_recruits.df %>% ggplot(aes(x = return_year, y = oso_origin_nat_returns/1000))+
  geom_line(linewidth = 0.5)+
  geom_point(size = 2)+
  geom_segment(aes(x = 1977,   xend = 2007.5, y = pre_fwmt,  yend = pre_fwmt),  colour = "red",       linewidth = 1.0, linetype = 3)+ # CB: this is for the option of adding the pre- and current freshwater management tools era to compare mean recruits between the two time periods
  geom_segment(aes(x = 2007.5, xend = 2021,   y = post_fwmt, yend = post_fwmt), colour = "darkgreen", linewidth = 1.0, linetype = 3)+
  geom_hline(yintercept = mean(oso_recruits.df$oso_origin_nat_returns)/1000,    colour = "blue",      linewidth = 1.0, linetype = 2)+
  scale_y_continuous(breaks = c(0, 100, 200, 300, 400),
                     minor_breaks = c(50, 150, 250, 350))+
  scale_x_continuous(breaks = c(seq(from = 1976, to = last_year, by = 4)))+
...
# juv_dat <- read_excel("../data/SIS - Data - OSOYOOS - 24.05.13.xlsx", sheet = "Juvenile Data") #presmolt (was 24.01.12.xlsx) [hs: 240426]
# #Pre-smolt abundance
# j_dat <- juv_dat[c(3:nrow(juv_dat)),c(2,6)] %>% #grabbing the appropriate columns from the excel file
#   dplyr::select(s_year = `Smolt Migration Year`, no_smolt = `...6`) %>% 
#   mutate_at("no_smolt", as.numeric) %>% #selecting columns and converting no_smolt to numeric
#   filter(s_year >=1998, s_year <= last_year)                                                    
# # write.csv(j_dat,  "../data/juv_dat_xlsx_240516.csv")    # if juv data imported, re-write to CSV  # comment out for GitHub

j_dat <- read.csv("../data/juv_dat_xlsx_240516.csv")    # replaces import data from SIS-OSO xlsx workbook # keep for GitHub [240510]
...
#radat <- left_join(recruit %>% dplyr::select(ret_year, oso_sox_mouth = recruit_os), a_dat, by = "ret_year") %>% #joining the recruitment dataframe with the age composition data frame ***NOTE that oso_sox mouth here is the number of osoyoos recruits, not the total number of sockeye of all stocks below Bonneville
radat  <- left_join(oso_recruits.df %>% dplyr::select(return_year, oso_sox_mouth = oso_origin_nat_returns), a_dat, by = "return_year") %>% #joining the recruitment dataframe with the age composition data frame ***NOTE that oso_sox mouth here is the number of osoyoos recruits, not the total number of sockeye of all stocks below Bonneville
  mutate(sox_age_fry  = oso_sox_mouth * ac_fryr, #calculating the number of recruits according to each age composition source
         sox_age_sire = oso_sox_mouth * ac_sire,
         sox_age_best = oso_sox_mouth * ac_best) %>% 
  group_by(s_year) %>% summarize(oso_sox_fry = sum(sox_age_fry), #summing the recruits by smolt year
                                 oso_sox_sire = sum(sox_age_sire),
                                 oso_sox_best = sum(sox_age_best)) %>% 
  left_join(., j_dat, by = "s_year") %>% #joining the juvenile dataframe
  filter(s_year > 1997) %>% #removing all years prior to 1998
  mutate(fry_surv = oso_sox_fry / no_smolt, #calculating marine survival
         sire_surv = oso_sox_sire / no_smolt,
         best_surv = oso_sox_best / no_smolt) %>%
  dplyr::select(s_year, fry_surv, sire_surv, best_surv) %>% #streamlining the dataset to the relevant columns
  pivot_longer(cols = ends_with("_surv"), #arranging the data to long format for plotting
               names_to = "source",
               values_to = "survival")

#plotting marine survival
ggplot(radat, aes(x = s_year, y = survival, colour = source))+
  ...
  labs(x = "Smolt year", y = "Survival (proportion)")+
  geom_hline(yintercept = sum(radat[radat$source == "best_surv", 3])/nrow(radat[radat$source == "best_surv", 3]), colour = "red", alpha = 0.5)+
