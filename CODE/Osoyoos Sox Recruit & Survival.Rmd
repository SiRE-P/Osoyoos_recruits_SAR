...
oso_recruits.df <- oso_recruits %>% 
  ...
  mutate(wenatchee = round(columbia - okanagan)) %>%                                                           # Wen-bound SK at mouth of Columbia
  ...
  mutate(ska_total_returns = ifelse(return_year == 2013, ska_origin_nat_returns + 0.5 * hatchery_returns,      # SKA total = H-O + N-O (presuming 50% of H-O returns --> SKA in RY2013 (since half or BY2009 releases were in SKA and half in OSO lake...
...
# mean before and after FWMT implementation - note, 2008 is first year where we might expect to see difference in adult returns  
pre_fwmt  <- mean(oso_recruits.df[oso_recruits.df$return_year %in% c(1977:2007),]$oso_origin_nat_returns)/1000
post_fwmt <- mean(oso_recruits.df[oso_recruits.df$return_year %in% c(2008:last_year),]$oso_origin_nat_returns)/1000 ### compute geometric mean ###
...
filename <- paste("../figures/Age_Composition_", timestamp, ".png", sep = "")   # filename for figure output 
ggsave(file = filename, width = 6, height = 4, units = "in")                    # saves the plot to filename
...
#saving the plot
filename <- paste("../figures/OSO_SAR_Survival_", timestamp, ".png", sep = "")   # filename for figure output 
ggsave(file = filename, width = 6, height = 4, units = "in")                     # saves the plot to filename
