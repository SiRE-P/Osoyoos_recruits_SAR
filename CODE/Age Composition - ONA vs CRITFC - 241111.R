...
##    *NEW* [241025] Code trims age>2.3 from CRITFC annual summary
##          data proportions, and recalculates age 1.1 to 2.3 proportions (since re-reading of a sample of these 2002-2003
##         "older age" scales indicates they were mis-read, and should be 1.2s and 1.3s, pers. comm. JFryer, email 240614).
##          This enables Recruits.R code to entirely apply CRITFC age comp as the preferred age composition (and ignore ONA
##          Spawning Ground age composition). MAINLY AFFECTS RY2002 and 2003.
##    #NEW# [241111] Code NOW reads revised age composition data, re-analyzed from raw data supplied by JF (CRITFC) after
##          omission of older age classes (3.1-4.2) - presumed erroneous - in order to obtain less biased age comp summary.
##          See workbooks OKA Returns at Age from JF 24.11.11.xlsx and CRITFC 2002 and 2003 Sockeye data.xlsx in directory
##          C:\DFO-MPO\OneDrive\OneDrive - DFO-MPO\okanagan_data\age_composition for details.
##        - Code then compares "past" annual ocean age comp assignment (Hyatt & Stockwell) with a "CRITFC-preferred"
...
# select(year, age_1.1, age_1.2, age_1.3, age_1.4, age_2.1, age_2.2, age_2.3, thermal_mark)  # no age 2.3s in spawning ground data!
...
## 241025   Adjusted CRITFC age comps to REMOVE age 3.1 AND OLDER FISH PERCENTS
##          and recalculate percent at age. Mainly affects 2002 & 2003, where
##          age 3.1+ amount to 10-15% of all age classes, but Jeff checked some of
##          these scales and found they were mis-read, and should be 1.2s and 1.3s.
##          HOWEVER Jeff did not finish the scale reading, and has been asked for
##          the raw data for re-analysis, but so far no response (241025). THUS,
##          code below reanalyzes (redistributes) age comp for regular age classes
##          based on annual percentages trimmed to <= age 2.3.
## ------------------------------------------------------------------------------
...
# age_comp_critfc <- read_xlsx("C:\\DFO-MPO\\OneDrive\\OneDrive - DFO-MPO\\Sockeye Index Stocks\\Okanagan\\Harvest\\OKA Returns at Age from JF 24.08.26.xlsx",
  age_comp_critfc <- read_xlsx("C:\\DFO-MPO\\OneDrive\\OneDrive - DFO-MPO\\okanagan_data\\age_composition\\OKA Returns at Age from JF 24.11.11.xlsx",
                                                       sheet = "CRITFC Age Comp", na="")   # read CRITFC age comp from workbook in OneDrive ### fix folder spec
filename <- here("DATA", "age_comp_critfc_241111.csv")
...
age_comp_critfc_metadata <- age_comp_critfc %>%                                 # retain meta-data by year for later merge
  dplyr::select(return_year = Year, Sample_Size, Source, Age_Method, Notes)
                
age_comp_critfc_char <- age_comp_critfc %>%    
  rename(return_year = Year) %>%
  dplyr::select(-Other,-Confirmed,-Source,-Notes,-X,-Sample_Size) %>%           # remove meta-data columns
  mutate(source = "CRITFC")                                                     # add source data tag
  
age_comp_critfc_numeric <- age_comp_critfc_char %>%
  dplyr::select(-TOTAL, -Age_Method, -source)
  
age_comp_critfc_numeric[] <- lapply(age_comp_critfc_numeric, function(x) as.numeric(as.character(x)))
age_comp_critfc_numeric[] <- lapply(age_comp_critfc_numeric, function(x) ifelse(is.na(x), 0, x))

age_comp_critfc_age_fix <- age_comp_critfc_numeric %>%
  dplyr::select(-prop_age_3.1, -prop_age_3.2, -prop_age_3.3, -prop_age_4.1) %>%
  rowwise() %>% 
  mutate(total = sum(prop_age_1.1, prop_age_1.2, prop_age_1.3,                  # Using only proportions for age 1.1 to 2.3
                     prop_age_2.1, prop_age_2.2, prop_age_2.3)) %>%             # Calculate the sum of N for each row
  mutate(prop_age_1.1 = prop_age_1.1 / total) %>%
  mutate(prop_age_1.2 = prop_age_1.2 / total) %>%
  mutate(prop_age_1.3 = prop_age_1.3 / total) %>%
  mutate(prop_age_2.1 = prop_age_2.1 / total) %>%
  mutate(prop_age_2.2 = prop_age_2.2 / total) %>%
  mutate(prop_age_2.3 = prop_age_2.3 / total) %>%
  mutate(total = sum(prop_age_1.1, prop_age_1.2, prop_age_1.3,
                     prop_age_2.1, prop_age_2.2, prop_age_2.3))                 # ReCalculate the sum of N for each row

age_comp_critfc_wide <- merge(age_comp_critfc_age_fix, age_comp_critfc_metadata,
                              by = c("return_year"), all=TRUE)   

filename <- here("DATA", paste("age_comp_critfc_wide_", timestamp, ".csv"))
write.csv(age_comp_critfc_wide, filename)                                       # saves the data to filename
  
age_comp_CRITFC_long <- age_comp_critfc_wide %>% # age_comp_critfc_wide %>%     # pivot to list proportions by return year and age class
  ...
  dplyr::select(-Source, -Notes, -Age_Method, -total)
...
# best_age_comp_source_CRITFC$best_source =                                     # obsolete assignments (241025)
...
best_age_comp_source_CRITFC$best_source =
  ifelse(best_age_comp_source_CRITFC$return_year %in% c(1985,1987:last_year), "CRITFC", # revised to assign CRITFC to all years 241025
         best_age_comp_source_CRITFC$best_source)                     
best_age_comp_source_CRITFC$best_source =
  ifelse(best_age_comp_source_CRITFC$return_year %in% c(1986,1980:1984), "NA",     # and missing years to NA
         best_age_comp_source_CRITFC$best_source)                     
...
best_age_comp_source_ONA$best_source =
  ifelse(best_age_comp_source_ONA$return_year >= 2001, "ONA",                   # assign ONA as 'best source' for all years >=2001 (was 2000, but removal of summer 'deadpitch' samples (241025) left no ONA age comp for 2000 spawners)
         best_age_comp_source_ONA$best_source)                                  # to use Spawn Grd data where available...
best_age_comp_source_ONA$best_source =
  ifelse(best_age_comp_source_ONA$return_year %in% c(1986,1980:1984), "NA",     # and missing years to NA
         best_age_comp_source_ONA$best_source)                     

filename = here("DATA", "best_age_comp_source_ONA_241025.csv")
...
age_comp_source_all <- merge(age_comp_ONA_CRITFC, 
  ...
  mutate(ocean_age = str_sub(age_class, start = -1))                            # Simple calculation of ocean age (years) from decimal of age class
...
filename <- here("FIGURES", paste("Diff_in_OceanAge_1_Comp_", timestamp, ".png", sep=""))
ggsave(file = filename, width = 8, height = 4, units = "in")                    # save for report
...
filename <- here("FIGURES", paste("Diff_in_OceanAge_2_Comp_", timestamp, ".png", sep=""))
ggsave(file = filename, width = 8, height = 4, units = "in")                    # save for report
...
filename <- here("FIGURES", paste("Diff_in_OceanAge_3_Comp_", timestamp, ".png", sep=""))
ggsave(file = filename, width = 8, height = 4, units = "in")                    # save for report
